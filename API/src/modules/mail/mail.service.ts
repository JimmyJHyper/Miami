...
export class MailService {
  ...
  public async sendRentRequestToOwner(
    ...
    const accessories = pricing.gearCost.gearCost.map((g) => ({
      ...
      hasDuration:g.title!='Unlimited Miles'
    }));
  ...
  public async sendRentRequestToClient(
    ...
    const mailConfig: MailOptions = {
      ...
      // to: "jimmyjindyps@gmail.com", 
      template: EmailTemplates.RENT_REQUEST_TO_CLIENT,
      subject: 'Motorcycle Booking Confirmed',
      attachments: [
        {
          path: bikeOrder.bikeImage,
          cid: 'bike-image',
  ...
  public async sendResetPassword(user: User) {
    const mailConfig: MailOptions = {
      ...
      subject: 'Miami Motorbike Rental - Reset password',
      context: {
        name: `${user.firstName} ${user.lastName}`,
        verificationLink: `${environment.frontEndUrl}/reset-password/${user.resetCode}`,
      },
    };
    await this.sendEmail(mailConfig);
  }
