...
export class StripeService {
  ...
  async createPaymentIntent(
    ...
    console.log("coupooooooooooooooooonCode:",orderDetails.couponCode)
const coupon:Coupon = await this.couponService.byCode(orderDetails.couponCode)?? emptyCoupon
  ...
  async refundPaymentIntent(intentId: string) {
    try {
      // Fetch the PaymentIntent to determine its status
      const paymentIntent = await this.stripe.paymentIntents.retrieve(intentId);
  
      if (paymentIntent.status === "requires_capture") {
        // Cancel the PaymentIntent
        return await this.cancelPaymentIntent(intentId);
      } else if (paymentIntent.status === "succeeded") {
        // Refund the PaymentIntent
        return await this.refundCapturedPaymentIntent(intentId);
      } else {
        console.log(`PaymentIntent status is ${paymentIntent.status}. No action taken.`);
        return false;
      }
    } catch (error: any) {
      console.error("Error handling PaymentIntent:", error.message);
      return false;
    }
  }

  async refundCapturedPaymentIntent(intentId: string) {
    try {
  ...
  async cancelPaymentIntent(intentId: string) {
    try {
      const cancelResponse = await this.stripe.paymentIntents.cancel(intentId);
      console.log("PaymentIntent cancelled:", cancelResponse);
      return true;
    } catch (error: any) {
      console.error("Error cancelling PaymentIntent:", error.message);
      return false;
    }
  }
