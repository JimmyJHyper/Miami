...
class BikePriceCalculation {
  ...
  public calculate(
    ...
    const rentalCost = {
      rate: Number(((price * (1 - discount))*(1-(couponPer/100))).toFixed(2)),
      discount,
      total: Number(((price * (1 - discount) * days)*(1-(couponPer/100))).toFixed(2)),
    };
  ...
  getGearCost(gear: Set<GearTypes>, days: number) {
    ...
    const gearCost = gearArr.map((gearType) => ({
      ...
      subtotal: gearPrices[gearType].hasDuration == true ? (gearPrices[gearType].price * days) : gearPrices[gearType].price,
    }));
