...
export class BikeGetResponseDto extends BaseResponseDto {
  ...
  @Transform((bike) => {
    try {
      return parseWpAttachmentMetadata(bike?.obj?.highlights)
    } catch (error) {
      return bike?.obj?.highlights
    }
  })
  highlights;
  ...
  @Transform((bike) =>{
    try {
      return parseWpAttachmentMetadata(bike?.obj?.features)
    } catch (error) {
      return bike?.obj?.features
    }
  })
  features;
  ...
  @Transform((bike) =>{
    try {
      return parseWpAttachmentMetadata(bike?.obj?.extras)
    } catch (error) {
      return bike?.obj?.extras
    }
  } )
  extras;
