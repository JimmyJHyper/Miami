...
import {  PTitle } from "../common/styles";
import {
  ButtonResend,
  ButtonSubmit,
  FormWrapper,
...
import { requestEmailVerification } from "@/apis/usersApis";
import { DivFormWrapper } from "../common/styles";
function RegisterVerifyEmail({callback,email}:{callback: Function,email:string}) {
  const {
    verifyOrder,
    registerValues,
    orderId,
  ...
  const validateCodeValue = (value: string) => {
    ...
    if (String(value).length > 4) {
      setVerifyCodeError("The code should contain maximum of 4 digits.");
      return true;
    }
    if (String(value).length < 4) {
      setVerifyCodeError("The code should contain minimum of 4 digits.");
      return true;
    }
    setVerifyCodeError("");
    return false;
